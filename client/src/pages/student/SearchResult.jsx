    ...
    return (
        <div className='flex flex-col md:flex-row md:w-auto  items-start md:items-center border-b border-gray-300 py-4 gap-4'>
            <Link to={`/course-details/${course?._id}`} className='flex flex-col md:flex-row gap-4 w-full md:w-auto '>
                <img src={course?.courseThumbnail} alt="Course Thumbnail" className='h-40 w-full md:w-75 object-cover rounded' />
            </Link>
            <div className='flex flex-col gap-2'>
                <h1 className='font-bold text-xl'>{course?.courseTitle}</h1>
                <p className='font-semibold text-gray-800 dark:text-gray-300 truncate '>{course?.courseSubTitle}</p>
